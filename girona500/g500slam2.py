...
from slam_worker import PHDSLAM
import numpy as np
...
class G500_SLAM():
    ...
    def init_slam(self):
        slam_worker = PHDSLAM()
        slam_worker.set_parameters(np.eye(3)*self.config.model_covariance, 
        ...
        if nparticles == 2*ndims+1:
            ...
            slam_worker.vehicle.states = sigma_pts(np.zeros((1, ndims)), 
                                                   sc_process_noise, 
                                                   _alpha=UKF_ALPHA, 
                                                   _beta=UKF_BETA, 
                                                   _kappa=UKF_KAPPA)[0]
    ...
    def updateGps(self, gps):
        ...
        (gps.longitude_hemisphere >= 0):
            ...
            if not config.gps_data :
                ...
                est_state = slam_estimate.vehicle.ned.state[0, 1]
    ...
    def update_imu(self, imu):
        ...
        if not config.imu_data :
            ...
            config.tast_time.imu = copy.copy(imu.header.stamp)
...
def main():
    try:
        ...
        if not __PROFILE__:
            ...
            #girona500_navigator.update_imu(imu_msg)
            if 1: #girona500_navigator.config.gps_update :
                try:
                    ...
                    for count in range(__PROFILE_NUM_LOOPS__):
                        ...
                        girona500_navigator.updateGps(gps_msg)
            ...
            for count in range(__PROFILE_NUM_LOOPS__):
                ...
                girona500_navigator.update_svs(svs_msg)
            ...
            for count in range(__PROFILE_NUM_LOOPS__):
                ...
                girona500_navigator.update_dvl(dvl_msg)
